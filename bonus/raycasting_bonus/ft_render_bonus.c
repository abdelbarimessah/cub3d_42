...
/*   Updated: 2023/05/27 22:45:18 by aadnane          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	map->img_ptr_mini = mlx_new_image(map->mlx, (map->maxlinelenght * \
		TILE_SIZE), map->number_line * TILE_SIZE);
	map->img_addr_mini = (int *)mlx_get_data_addr(map->img_ptr, \
		&map->bits_per_pixel, &k, &map->endian);
}
